...
async def test_a_transient_failure_surfaces_as_itself_and_keeps_the_store() -> None:
    ...
    handle = FakeHandle(_sequence(DriverError(HICCUP), {"ok": True}))
...
async def test_only_a_dead_context_renders_the_mandated_stale_uid_string() -> None:
    dead = FakePage(FakeHandle(_sequence(DriverError(DEAD_CONTEXT))))
    with pytest.raises(ValueError) as staled:
        await element_call(dead, "resolve", "e7", {})
    assert str(staled.value) == STALE_UID.format(uid="e7")

    hiccup = FakePage(FakeHandle(_sequence(DriverError(HICCUP))))
    with pytest.raises(DriverError) as raised:
...
async def test_only_a_dead_context_throws_the_uid_namespace_away() -> None:
    dead_handle = FakeHandle(_sequence(DriverError(DEAD_CONTEXT)))
    dead = FakePage(dead_handle, FakeHandle(_sequence({"ok": True})))
    ...
    hiccup_handle = FakeHandle(_sequence(DriverError(HICCUP), {"ok": True}))
    hiccup = FakePage(hiccup_handle)
...
async def test_a_driver_timeout_is_not_element_staleness() -> None:
    ...
    handle = FakeHandle(_sequence(DriverTimeout("Timeout 30000ms exceeded.")))
    page = FakePage(handle)
...
async def test_a_closed_tab_is_re_raised_by_type() -> None:
    ...
    page = FakePage(FakeHandle(_sequence(PLAYWRIGHT_TARGET_CLOSED_ERROR())))
...
async def test_a_page_that_never_answers_reports_the_operation_budget() -> None:
    async def never(_payload: dict[str, Any]) -> Any:
        await asyncio.sleep(3600)

    page = FakePage(FakeHandle(never))
...
async def test_a_wedged_dispose_does_not_wedge_the_whole_tab(
    ...
    page = FakePage(
        FakeHandle(_sequence(DriverError(DEAD_CONTEXT)), hang_dispose=True),
        FakeHandle(_sequence({"ok": True})),
    )
...
_OP_CASES: list[tuple[str, dict[str, Any], Callable[[FakePage], Any]]] = [
...
async def test_an_internal_failure_names_the_operation_that_broke(
    op: str, replies: dict[str, Any], run: Callable[[FakePage], Any]
) -> None:
    """A bug in our own page script is the one case where the op name is everything."""
    page = FakePage(FakeHandle(_per_op(replies)))
