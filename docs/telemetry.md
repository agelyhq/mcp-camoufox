...
config snapshot (proxy redacted to scheme and host), and a
`session_closed(reason=shutdown)` per profile at shutdown.
...
That last one is why nothing has been removed from the tool surface yet: the
measurement had to exist before the decision. See [decisions.md](decisions.md).
